# ToolErrorMiddleware 是 langchain 1.3.14 才新增的，這裡原封不動搬過來，
# 給裝了較舊版本 langchain 的環境當備用（見 langchain/agents/middleware/tool_error.py）。
# 版本判斷邏輯在 agents/middleware/__init__.py：langchain 夠新會自動改用官方版本，
# 不需要手動切換，這個檔案也不用刪。
...
# AgentMiddleware 這些基礎類別在 1.3.11 就有了，不用跟著複製，直接從 langchain 匯入即可，
# 只有 ToolErrorMiddleware 本身是這個版本沒有、才需要複製到這個檔案裡。
